        ...
        const seguir = await confirmar({
          ...
          texto: (
            <>
              ...
              <p className="text-[13px] text-tinta-suave">Lo más seguro es cambiar primero la imagen donde aparece y eliminarla después.</p>
            </>
          ),
        });
        if (seguir) {
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center gap-1.5 rounded-2xl border-2 border-dashed px-4 py-7 text-center transition-colors ${
          arrastrando ? 'border-marca bg-marca/10' : 'border-borde-fuerte bg-tinta/[0.02] hover:border-marca/60 hover:bg-marca/5'
        }`}
      >
        <Upload size={22} className={arrastrando ? 'text-marca' : 'text-tinta-tenue'} />
        ...
        <p className="text-[12px] text-tinta-suave">Fotos (JPG, PNG, WebP), PDF y vídeo MP4. Las fotos se optimizan solas.</p>
        <input
          ref={inputRef}
          type="file"
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[12rem]">
          <Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-tinta-tenue" />
          <input
            value={busqueda}
      ...
      {cargando ? (
        ...
        <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-tinta-suave">
          <ImageOff size={28} />
...
const Ficha: React.FC<{
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border border-borde bg-panel">
      <div className="relative aspect-[4/3] bg-lienzo">
        {medio.tipo === 'imagen' ? (
          <img src={medio.url} alt={medio.alt} loading="lazy" className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full w-full flex-col items-center justify-center gap-1.5 text-tinta-tenue">
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-0 transition-opacity group-hover:opacity-100">
          {modoSelector && (
            <button onClick={onUsar} className="oc-pulsable rounded-lg bg-marca px-2.5 py-1 text-[12px] font-bold text-tinta hover:bg-marca-oscuro">
              <Check size={12} className="mr-1 inline" />Usar esta
            </button>
          )}
          {propio && (
            <>
              <button onClick={onEditar} title="Cambiar nombre y descripción" className="oc-pulsable rounded-lg bg-panel/90 p-1.5 text-tinta hover:bg-panel"><Pencil size={13} /></button>
              <button onClick={onReemplazar} title="Sustituir el archivo" className="oc-pulsable rounded-lg bg-panel/90 p-1.5 text-tinta hover:bg-panel"><RefreshCw size={13} /></button>
              <button onClick={onEliminar} title="Eliminar" className="oc-pulsable rounded-lg bg-panel/90 p-1.5 text-red-600 hover:bg-panel"><Trash2 size={13} /></button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="px-2.5 py-2">
        <p className="truncate text-[12.5px] font-bold text-tinta" title={medio.nombre}>{medio.nombre}</p>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        {error && <Aviso tipo="error">{error}</Aviso>}
        {medio.tipo === 'imagen' && (
          <img src={medio.url} alt="" className="max-h-48 w-full rounded-xl object-contain bg-lienzo" />
        )}
        <div>
